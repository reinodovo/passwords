...
WordList shuffle(std::mt19937 rng) {
  WordList all(ALL_WORDS);
  for (int i = 0; i < ALL_WORDS; i++) all[i] = all_words[i];
  for (int i = ALL_WORDS - 1; i >= 0; i--) {
    int j = rng() % (i + 1);
...
WordList generate_words(int code) {
  std::mt19937 rng(code);
  WordList words = shuffle(rng);
  return WordList(words.begin(), words.begin() + WORD_COUNT);
}